...
impl<Resamp> Sampler<Resamp> where Resamp: Resampler {
    fn advance(&mut self, mut interval: usize, sample_data: &SampleData) {
        let loop_points: (isize, isize) = tuple_isize(sample_data.loop_points.unwrap_or((0, sample_data.length - 1)));
    ...
    fn check_resample(&mut self, samplerate: usize, sample_data: &SampleData) {
        if samplerate != self.last_rate || self.last_rate == 0 {
            self.resampled = StereoBuffer::new(sample_data.length * samplerate / sample_data.samplerate);
            StereoBuffer::from(&sample_data.data).copy_to_and_resample((&mut self.resampled).into(), &self.resampler);
        }
    }
...
impl<Resamp> SourceObj for Sampler<Resamp> where Resamp: Resampler {
    fn gush_audio(&self, to: &mut StereoBuffer) {
        self.play_buffer.copy_to_and_resample(to.into(), &NoResample);
    }
}
